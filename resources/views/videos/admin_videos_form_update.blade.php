@extends('layouts.mainLayoutSession')
  ...
  @section('title') VIDEOS - MODIFICAR VIDEO  @endsection

  @section('extraheader')
    <link rel="stylesheet" href="{{ asset('css/resources/videos_admin.css') }}">
  @endsection

  @section('in_page_title') MODIFICAR VIDEO  @endsection
    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-12 mainContainer">
          ...
          <form name="formVideo" id="formVideo" action="{{ url('/Admin_Videos/upd_video/save') }}" method="post" enctype="multipart/form-data">
            {!! csrf_field() !!}
              ...
              <div class="col-sm-6 form_sub_block">
                <div class="row">
                  <div class="col-sm-4 text-center">
                    <img src="{{ asset('img/web/videos/'.$data['thumbnail']) }}" class="thumb_preview" id="thumb_preview">
                  </div>
                  <div class="col-sm-8 progress_block">
                    <div class="row">
                      ...
                      <div class="col-sm-12">
                        <img src="{{ asset('img/web/admin_elements/sound_on.png') }}" alt="" class="img_lock" id="preview_video"> <span class="span_lock" id="video_span">Video actual: {{ $data['url'] }}</span>
                      </div>
                      <div class="col-sm-12">
                        <img src="{{ asset('img/web/admin_elements/'.(($data['lock'] == 0)? 'lock_off.png' : 'lock_on.png')) }}" alt="" class="img_lock" id="img_lock"> <span class="span_lock" id="lock_span">{{ (($data['lock'] == 0)? 'Contenido libre para cualquier usuario' : 'Contenido para usuarios premium') }}</span>
                      </div>
                    </div>
                  </div>

                  <input type="hidden" name="id_video" id="id_video" value="{{ $data['id'] }}">
                  ...
                  <input type="checkbox" name="premium_lock" id="premium_lock" value="1" style="display:none;" {{(($data['lock'] == 0)? '' : 'checked')}}>

                  <div class="col-sm-12 form_block_element" style="background-image: url('{{ asset('img/web/admin_elements/titulo.png') }}')">
                    <input type="text" name="title" class="form-control" placeholder="Titulo" id="title" value="{{$data['title']}}" required>
                  </div>

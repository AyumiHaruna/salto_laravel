...
$( "#submitAddPermissionForm" ).click(function() {
        $( "#addPermissionForm" ).submit();
});

$('.openEditPermissionModal').click(function(e)
    {
    ...
    $('#permissions_edit').multiSelect('deselect_all');

    $.ajax({
        headers : {'X-CSRF-TOKEN': token },
        dataType: "json",
        data    : { uid: uid },
        url     : route_permisos_get_permiso,
        type    : 'GET',
        ...
        success: function (msg) {
            ...
            if(msg.status == 200)
            {
                ...
                $('#formDescription').val(msg.role.description);

                msg.role.permissions.forEach( function(permission){
              //      $('option.edit_permission_option[value='+permission.id+']').attr("selected",true);
                    $('#permissions_edit').multiSelect('select', [''+permission.id]);
                });
